...
void Proximity(int ind, const std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& processed, KdTree* tree, float distanceTol)
{
    ...
    std::vector<int> nearbyPointsInd = tree->search(points[ind], distanceTol);
    for(int indices: nearbyPointsInd)
    {
        if (processed[indices] == false)
            Proximity(indices, points, cluster, processed, tree, distanceTol);
    }
}
